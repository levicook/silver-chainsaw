...
The basic idea is it routes funds continuously to an unlimited number of addresses, according to a set of percentage allocations.


The winning bids from these three auctions will be split amongst the musicians, visual artists, project operatives, and Songcamp’s treasury. We are using Mirror’s Splits tool in order to instantaneously and trustlessly split all auction proceeds to their rightful recipient. To see this breakdown in action, scroll down to the Split blocks below. " how do the winning bids work exactly? Is it something like top 10 bids get a share of the upside?
